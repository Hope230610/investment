...
def upgrade() -> None:
    # === 1. 创建枚举类型 ===
    op.execute(f"CREATE TYPE {INTERACTION_SCENARIO} AS ENUM ('single_stock_check', 'pre_trade_check', 'post_trade_review')")
    ...
    op.create_table(
        ...
        sa.Column("scenario", sa.Enum("single_stock_check", "pre_trade_check", "post_trade_review", name=INTERACTION_SCENARIO), nullable=False),
        sa.Column("status", sa.Enum("processing", "partial_ready", "ready", "expired", "failed", name=ANALYSIS_STATUS_ENUM), nullable=False, server_default="processing"),
    ...
    op.create_table(
        ...
        sa.Column("valid_period", sa.Enum("short", "medium", "long", name=VALID_PERIOD_ENUM), nullable=False, server_default="medium"),
    )
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(), nullable=False, server_default=sa.text("NOW()")),
        sa.Column("behavior_type", sa.Enum("chasing_rise", "panic_sell", "frequent_trading", name=BEHAVIOR_TYPE_ENUM), nullable=False),
        sa.Column("severity", sa.Enum("low", "medium", "high", name=SEVERITY_LEVEL_ENUM), nullable=False),
        ...
        sa.Column("action_taken", sa.Enum("continued", "delayed", "cancelled", "logged_only", name=INTERVENTION_ACTION_TAKEN_ENUM), nullable=True),
    )
...
def _migrate_analyses_data():
    ...
    op.execute("""
        ...
        SELECT
            ...
            created_at,
            updated_at,
            LOWER(scenario)::interactionscenario,
            LOWER(status)::analysisstatusv2,
            NULL,  -- user_profile_snapshot 历史数据不可用
            scenario_payload,
            CASE WHEN status IN ('ready', 'expired', 'failed') THEN updated_at ELSE NULL END,
            CASE WHEN status IN ('ready', 'expired') THEN updated_at ELSE NULL END,
...
def downgrade() -> None:
    ...
    op.execute(f"DROP TYPE IF EXISTS {INTERACTION_SCENARIO}")
