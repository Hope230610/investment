...
当前最新验收快照（2026-05-04）：
- 后端 `pytest -q`：76 passed。
- 前端 `npm run test -- --run`：48 passed。
- `npm run build`：passed，仍有已知 500KB chunk 提示。
- Mock 行情 Full E2E：21 passed。
- 真实行情 Full E2E：21 passed。
- 已补充统一错误响应契约测试，覆盖请求体验证错误和鉴权错误。
...
- 结果页失败状态必须展示结构化下一步：失败原因、可重试动作、可检查的数据来源/股票标识，以及当前占位结论的失效边界。
