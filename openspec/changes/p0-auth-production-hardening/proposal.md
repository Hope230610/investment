...
当前真实代码仍存在未认证业务访问被开发态便利逻辑放行、运行时测试身份被隐式注入、生产关键安全配置仍允许默认值兜底等问题。这些行为与 `auth-session`、`platform-contracts` 已定义的共享访问边界和安全治理底线冲突，必须先收紧真实环境门槛，后续 API 契约收口和联调才有稳定前提。

## What Changes

- 强化 `auth-session`，明确受保护业务访问必须 fail-closed，不允许未认证请求、无效会话或隐式测试身份继续进入业务处理流程。
- 强化 `auth-session`，要求生产与真实内测环境在启动时显式拒绝任何绕过正式认证的便捷身份能力。
- 强化 `platform-contracts`，要求真实环境关键安全配置只能通过显式方式提供，不允许默认安全配置继续进入真实环境。
- 强化 `platform-contracts`，把未登录、会话失效、越权访问和日志脱敏要求纳入统一 contract，便于前后端和 QA 用同一套基线验收。
...
- 受影响代码区域：`investment-back/src/api/deps.py`、`investment-back/main.py`、`investment-back/src/core/config.py`、`investment-back/src/api/*`、`investment-front/src/api.ts`、前端登录态与错误处理相关页面/状态层
