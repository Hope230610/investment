...
观察列表的服务端真源为 `watchlists` 表（新版），不再是 `watchlist_items_legacy` 表（旧版）。`/api/v1/watchlist` 路由对应的 WatchlistService 路径为废弃路径，不再向前端新增调用。后续所有观察列表操作走 `POST /api/v1/analysis/{id}/record-reason` 写入（通过分析记录附带写入），读路径走 `GET /api/v1/analysis` 的 filtered 查询。

#### Scenario: 跨设备查看观察列表（服务端真源）
- **WHEN** 用户在设备 A 添加股票到观察列表，设备 B 登录同一账号
- **THEN** 设备 B 通过 `GET /api/v1/analysis` 查询 `scenario=watchlist` 记录，应能查到该股票（后续补读 API 时以新实现为准，本场景说明读路径将统一）

#### Scenario: 废弃路径不再产生新数据
- **WHEN** 任何前端页面调用 `/api/v1/watchlist`
- **THEN** 返回 404 或空列表（取决于 legacy service 实现），不得向 watchlist_items_legacy 表写入新数据
...
`POST /api/v1/analysis/{id}/record-reason` 为当前唯一的理由写入路径，写入后 `watchlists` 表的 `focus_reason` 字段必须可被查询。查询路径以 `GET /api/v1/analysis` 过滤或后续新增的 `/api/v1/watchlist` 新实现为准（不走 legacy WatchlistService）。
